...
from utils.utils_gpt2_xl import _apply_inv_perm_scale
...
def ob_groupcover(model, size=4):
    ...
    with torch.no_grad():
        for name, module in model.named_parameters():
            ...
            nw, ci, rnd, perm = _gpt2_ob_groupcover_apply_slice(w_tensor, device, dtype, size)
            module.copy_(nw)
...
def recover_groupcover(A_obf, A_pub, size=4, th=0.1, max_th=0.5, step=0.1):
    ...
    K_est, P2_est = recover_K_and_P2(A_obf, B_fix, cluster_index, th=0.15, step=0.3, max_th=4, size=size)
...
def attack_groupcover(model, pre_model, size=4, vic_model=None, th=0.15, max_th=0.5):
    set_seed()
    restore_permutation = {}
        ...
        for name, module in model.named_parameters():
            if _gpt2_obfus_weight(name):
                ...
                def recover_and_report(A_guess_arr, rp, cci):
                    ...
                    print(f"name: {name}")
                    if _gpt2_is_c_attn(name):
                        m_slices = module.detach().cpu().chunk(3, dim=1)
                        p_slices = pre_w.cpu().chunk(3, dim=1)
                        v_slices = vic_state[name].detach().cpu().chunk(3, dim=1) if vic_model is not None else [None] * 3
                        for tag, m_slice, p_slice, v_slice in zip(("q", "k", "v"), m_slices, p_slices, v_slices):
